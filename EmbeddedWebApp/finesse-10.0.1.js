...
(function (factory) {
    ...
    var Utilities = /** @lends finesse.utilities.Utilities */ {
        ...
        currentServerTimeMillis : function () {
            if (!finesse.container.Config.clientDriftInMillis) {
                finesse.container.Config.clientDriftInMillis = 0;
            }
            return (new Date()).getTime() + finesse.container.Config.clientDriftInMillis;
        },
        ...
        */
         ...
         * {
         *     isIE: {Boolean},
         *     version: {String}, // 8 for IE8, 9 or IE9
        ...
        detectIEBrowserVersion: function () {
            ...
            if (appName.indexOf("Microsoft Internet Explorer") !== -1) {
                ...
                } // Else unsupported IE browser version, return undefined for version
            }
        ...
        _objectDeleter: function (data) {
         ...
         _buildRefreshedCollection: function (data) {
            ...
            for (i = 0; i < dataArray.length; i += 1) {
                ...
                object = this._collection[objectId];
                if (object.explicitSubscription) {
         ...
         refresh: function() {
            ...
            this._doGET({
                success: function(rsp) {
                    ...
                    if (_this.explicitSubscription) {
                        _this._subscribeNode();
                    }
                },
                error: function(rsp) {
                    _this._errorNotifier.notifyListeners(rsp);
...
(function (factory) {
    ...
    var WorkflowAction = RestBase.extend({
        ...
        getActionVariables: function() {
            ...
            if(actionVariablesParent === null || actionVariablesParent.length === 0){
                return map;
            }
...
(function (factory) {
    ...
    var ClientLogger = ( function () { /** @lends finesse.cslogger.ClientLogger.prototype */
        var _hub, _logTopic, _originId, _sessId,
 
        /**
         * Get a short form (6 character) session ID from sessionStorage
         * @private
        */
        getSessId = function() {
            if (!_sessId) {
        ...
        {
            var date = new Date(), timeStr;
            timeStr = date.getFullYear().toString().substring(2) +
        ...
        */
        ...
        {
            var msg, timeStr = getDateTimeStamp(), sessKey=getSessId();
...
/*global window:true, gadgets:true, publisher:true, define:true, finesse:true, _tabTracker:true, _workflowActionEventTracker:true, frameElement:true, $:true, parent:true, MockControl:true */
...
(function (factory) {
    ...
    if ( typeof define === 'function' && define.amd )
    {
        define('containerservices/ContainerServices',["utilities/Utilities",
                ...
                "containerservices/WorkflowActionEvent"], factory );
    }
     ...
     * @param {String} topic : Specifices the callback to retrieve
    ...
    _topicCallback = function (topic) {
        ...
        switch (topic)
        {
            ...
            case finesse.containerservices.ContainerServices.Topics.WORKFLOW_ACTION_EVENT:
                ...
                break;
            default:
                throw new Error("ContainerServices : _topicCallback(topic) - Topic not recognized.");
        }
        return callback;
    },
    ...
    /**
     * Utility to be called within a gadget to determine the iframe of the parent container
     * corresponds to this gadget so that the gadget id can be parsed from the iframe id.
    ...
    _findMyGadgetId = function () {
        ...
        while (pWindow && pWindow.parent !== pWindow) {
            pWindow = pWindow.parent;
        }

        // Then, get the list of all gadget iframes
        gadgetFrames = pWindow.document.getElementsByClassName('gadgets-gadget');

        // Then check to see which iframe matches this window
        for (i = 0; i < gadgetFrames.length; i += 1) {
            if (gadgetFrames[i].contentWindow === self) {
                myIframe = gadgetFrames[i];
                break;
            }
        }

        // Parse the iframe id to get the gadget id
        if (myIframe) {
            iframeId = myIframe.id;
            if (iframeId.indexOf('finesse_gadget_') !== -1) {
                id = iframeId.substr(iframeId.indexOf('finesse_gadget_') + 15);
            }            
        }
...
(function (factory) {
    ...
    if (gadgets.Prefs) {
        ...
        authString = finesse.utilities.Utilities.getCookie("finesse_authorization");
